<template>
    <div class="menu">
        ...
        <a v-if="type == 'passenger'" @click="goPassenger()" :class="{'hover' : $route.name =='passengerRelease'}">发布信息</a>
        <a v-if="type == 'passenger'" v-link="{path : '/passenger/passengerSearch'}" :class="{'hover' : $route.name =='passengerSearch'}">找车</a>

        <a v-if="type == 'driver'" @click="goDriver()" :class="{'hover' : $route.name =='driverRelease'}">发布信息</a>
